...
const size_t config_ubx_item_count =
	sizeof(config_ubx_items) / sizeof(config_ubx_items[0]);

const char *const dynamic_models[] = {DYNAMIC_MODEL_ITEM_LIST(STRINGIFY)};
const size_t dynamic_models_items_count =
	sizeof(dynamic_models) / sizeof(dynamic_models[0]);
...
const size_t sample_rates_items_count =
...
uint8_t get_ubx_item_values(size_t index, strbf_t *sb) {
	switch (index) {
	case cfg_ubx_ubx_gnss:
		add_values_array(sb, gnss_desc, &gnss_desc_val[0],
						 gnss_desc_items_count, 0);
		return 1;
	case cfg_ubx_ubx_output_rate:
		add_values_array(sb, sample_rates, &sample_rate_values[0],
						 sample_rates_items_count, 0);
		return 1;
	case cfg_ubx_ubx_nav_mode:
		add_values_array(sb, dynamic_models, &dynamic_model_values[0],
						 dynamic_models_items_count, 0);
		return 1;
	default:
		return 0;
	}
...
static bool config_ubx_set_item_impl(size_t index, uint16_t val) {
	...
	case 0: { // ubx_output_rate (sample_rate)
		if (g_rtc_config.ubx.output_rate != val) {
			FUNC_ENTRY_ARGS(TAG,
					 "Sample rate config changed from %d to %"PRIu16", reinit UBX.",
	...
	case cfg_ubx_ubx_gnss: // gnss
		if (g_rtc_config.ubx.gnss != val) {
			FUNC_ENTRY_ARGS(TAG, "GNSS config changed from %"PRIu8" to %"PRIu16", reinit UBX.",
	...
	case cfg_ubx_ubx_nav_mode: // dynamic_model
		if (g_rtc_config.ubx.nav_mode != val) {
			FUNC_ENTRY_ARGS(TAG, "Dynamic model changed from %d to %"PRIu16", reinit UBX.",
	...
	bool ret = false;
	if (changed != 255) {
		uint8_t handle = config_ubx_handle();
...
bool config_ubx_set_item(size_t index, const char *value) {
	FUNC_ENTRY_ARGS(TAG, "index: %zu value: %s", index, value);
	if (!value) {
		return false;
	}
...
uint16_t config_ubx_get_next_value(size_t index) {
	FUNC_ENTRY_ARGS(TAG, "index: %zu", index);
	uint16_t new_value = 0;
	switch (index) {
	case cfg_ubx_ubx_output_rate: {
		// Cycle through sample rates: 1->2->5->10->16->20->1
		uint16_t current = g_rtc_config.ubx.output_rate;
		switch (current) {
		case 1:
			new_value = 2;
			break;
	...
	case cfg_ubx_ubx_gnss: {
		...
		default:
			new_value = 111;
			break; // Default to all systems
		}
		break;
	}
	case cfg_ubx_ubx_nav_mode: {
		// Cycle through nav modes:
		// Portable->Pedestrian->Automotive->Sea->Portable
		uint8_t current = g_rtc_config.ubx.nav_mode;
		switch (current) {
		case 0:
			new_value = 3;
			break; // Portable -> Pedestrian
		case 3:
			new_value = 4;
			break; // Pedestrian -> Automotive
		case 4:
			new_value = 5;
			break; // Automotive -> Sea
		case 5:
			new_value = 0;
			break; // Sea -> Portable
		default:
...
struct m_config_item_s *get_ubx_cfg_item(int num,
										 struct m_config_item_s *item) {
	// printf("num:%d", num);
	if (!item)
		return 0;
	if (num < 0 || num >= config_ubx_item_count)
		num = 0;
...
// if (needs_ubx_update && gps && gps->ubx_device &&
// gps->ubx_device->initialized) {
...
//             ubx_set_ggnss_and_rate(gps->ubx_device, ubx_gnss.value,
//             ubx_rate.value); ILOG(TAG, "[%s] set gnss: %d, rate: %d",
//             __func__, ubx_gnss.value, ubx_rate.value);
//         }
//     }
// }
// }
...
static int _get_next_cycled_idx(int current_idx) {
	...
	if (next_idx >= (int)config_ubx_item_count) {
		next_idx = 0;
	}
	return next_idx;
}

static config_module_registry_t ubx_module = {
	.name = "ubx",
	// .group_id = SCFG_GROUP_UBX,
	.flags = CONFIG_MODULE_FLAG_CYCLE_SCREENS,
	.parent_name = NULL,
	.nvs_key = NVS_KEY_UBX,
	.data_ptr = &g_rtc_config.ubx,
	.data_size = sizeof(cfg_ubx_t),
	.migrate_func = NULL,
	.ops =
		{
			...
			.get_next_value = config_ubx_get_next_value,
			.set_next_value = config_ubx_set_next_value,
...
void config_ubx_register(void) {
	ubx_module.handle = config_registry_register(&ubx_module);
	if (ubx_module.handle == CONFIG_MODULE_HANDLE_INVALID) {
		ELOG(TAG, "Failed to register UBX module");
	}
}

#if defined(CONFIG_LOGGER_COMMON_ENABLE_CFG_AUTOLOAD)
__attribute__((constructor)) static void config_ubx_auto_register(void) {
	config_ubx_register();
}
#endif

config_module_handle_t config_ubx_handle(void) { return ubx_module.handle; }
