...
const size_t config_screen_item_count =
	sizeof(config_screen_items) / sizeof(config_screen_items[0]);
const char *const config_speed_field_items[] = {
	SPEED_SCREEN_UPPER_FIELD_ITEM_LIST(STRINGIFY)};
const size_t config_speed_field_item_count =
	sizeof(config_speed_field_items) / sizeof(config_speed_field_items[0]);
...
const size_t screen_rotations_count =
	sizeof(screen_rotations) / sizeof(screen_rotations[0]);
...
bool get_screen_item_descriptions(size_t index, struct strbf_s *sb) {
	...
	case cfg_screen_bar_length:
		strbf_puts(sb,
				   "Bar length in meters used for distance bar calculations.");
		break;
...
uint8_t get_screen_item_values(size_t index, struct strbf_s *sb) {
	switch (index) {
	case cfg_screen_speed_field:
		add_values_array(sb, config_speed_field_items, 0,
						 config_speed_field_item_count, 0);
		return 1;
	case cfg_screen_board_logo:
...
bool config_screen_get_item(size_t index, config_item_info_t *info) {
	if (!info)
		return false;
	FUNC_ENTRY_ARGT(TAG, "index:%d", index);
	...
	case cfg_screen_bar_length:
		...
		sprintf(desc_buf, "%" PRIu32 " m", info->value);
		info->desc = desc_buf;
		info->min = 0;
		info->max = 10000;
		break;
...
static bool config_screen_set_item_impl(size_t index, uint16_t val,
	...
	case cfg_screen_full_refresh_interval:
		if (g_rtc_config.screen.full_refresh_interval != val) {
			FUNC_ENTRY_ARGSD(TAG, "EPD full refresh interval changed to %u",
							 val);
	...
	if (changed != 255) {
		uint8_t handle = config_screen_handle();
...
uint16_t config_screen_get_next_value(size_t index) {
	FUNC_ENTRY_ARGSD(TAG, "index: %zu", index);
	uint16_t new_value = 0, current = 0;
	switch (index) {
	case cfg_screen_speed_field: {
		// Cycle through speed field options
		current = g_rtc_config.screen.speed_field;
		new_value =
			(current >= config_speed_field_item_count - 1) ? 0 : current + 1;
		break;
	}
	case cfg_screen_board_logo: {
...
bool config_screen_set_next_value(size_t index) {
	FUNC_ENTRY_ARGSD(TAG, "index: %zu", index);
	return config_screen_set_item_impl(index,
									   config_screen_get_next_value(index), 0);
}
...
static int _get_next_cycled_idx(int current_idx) {
	...
	if (next_idx >= (int)config_screen_item_count) {
		next_idx = 0;
	}
	return next_idx;
}

static config_module_registry_t screen_module = {
	.name = "screen",
	// .group_id = SCFG_GROUP_SCREEN,
	.flags =
		CONFIG_MODULE_FLAG_DEFAULT_HIDDEN | CONFIG_MODULE_FLAG_CYCLE_SCREENS,
	.parent_name = NULL,
	.nvs_key = NVS_KEY_SCREEN,
	.data_ptr = &g_rtc_config.screen,
	.data_size = sizeof(cfg_screen_t),
	.migrate_func = NULL,
	.ops =
		{
			...
			.get_next_value = config_screen_get_next_value,
			.set_next_value = config_screen_set_next_value,
...
void config_screen_register(void) {
	screen_module.handle = config_registry_register(&screen_module);
	if (screen_module.handle == CONFIG_MODULE_HANDLE_INVALID) {
		ELOG(TAG, "Failed to register screen module");
	}
}

#if defined(CONFIG_LOGGER_COMMON_ENABLE_CFG_AUTOLOAD)
__attribute__((constructor)) static void config_screen_auto_register(void) {
	config_screen_register();
}
#endif

config_module_handle_t config_screen_handle(void) {
	return screen_module.handle;
}