...
#define CFG_FW_UPDATE_DEFAULTS() { \
    .version = 1, \
    .update_enabled = 1, \
    .update_channel = CFG_FW_UPDATE_CHANNEL \
}

#define CFG_ENUM_FW(l) cfg_fw_update_##l,
enum cfg_fw_update_item_e {
    CFG_FW_UPDATE_ITEM_LIST(CFG_ENUM_FW)
};
...
bool config_fw_update_set_next_value(size_t index); // Cycle to next value (for UI)
