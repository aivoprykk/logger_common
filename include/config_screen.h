...
#define NVS_KEY_SCREEN "screen_cfg"
...
#define CFG_SCREEN_ITEM_LIST(l)                                                \
	l(speed_field) l(board_logo) l(sail_logo) l(screen_rotation) l(bat_view)   \
		CFG_2ND_BUTTON_SCREEN_ITEMS(l) CFG_SPEED_SCREEN_LARGE_FONT_ITEMS(l)    \
			CFG_SCREEN_BRIGHTNESS_ITEMS(l) l(bar_length) l(sleep_info)

#define CFG_SCREEN_ALL_ITEM_LIST(l) CFG_SCREEN_ITEM_LIST(l)
...
#define CFG_SCREEN_DEFAULTS()                                                  \
	{                                                                          \
		...
		.bat_view = 2,                                                         \
		CFG_2ND_BUTTON_SCREENS CFG_SCREEN_SPEED_LARGE_FONT                     \
			SCREEN_BRIGHTNESS_DEFAULT.bar_length = 1852,                       \
		.sleep_info = "ESP GPS",                                               \
	}
...
#define SPEED_SCREEN_UPPER_FIELD_ITEM_LIST(l)                                  \
	l(dynamic) l(spd_10_sec) l(spd_alpha) l(spd_1852_m) l(spd_500_m)           \
		l(spd_dist_time) l(spd_max_2s_10s) l(spd_half_hour) l(spd_1_hour)      \
			l(spd_1h_dynamic)
...
#define SAIL_LOGO_ITEM_LIST(l)                                                 \
	l(GASails) l(Duotone) l(NeilPryde) l(LoftSails) l(Gunsails) l(Point7)      \
		l(Patrik)
...
typedef enum { BAT_VIEW_LIST(BAT_VIEW_ENUM) } bat_view_e_t;

#define CFG_ENUM_SCREEN(l) cfg_screen_##l,
enum cfg_screen_item_e { CFG_SCREEN_ALL_ITEM_LIST(CFG_ENUM_SCREEN) };
...
uint8_t config_screen_handle(void);
