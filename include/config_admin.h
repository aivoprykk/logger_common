...
#define NVS_KEY_ADMIN "admin_cfg"

#define SCFG_GRP_ADMIN(l) l(ADMIN)

// Admin configuration list
#define CFG_ADMIN_ITEM_LIST(l) l(admin_auth) l(admin_username) l(admin_password)
...
#define CFG_ADMIN_DEFAULTS()                                                   \
	{.version = 1, .admin_auth = 0, .admin_username = "", .admin_password = ""}

#define CFG_ENUM_ADMIN(l) cfg_admin_##l,
enum cfg_admin_item_e { CFG_ADMIN_ITEM_LIST(CFG_ENUM_ADMIN) };
...
config_module_handle_t config_admin_handle(void);
