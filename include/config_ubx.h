...
#define NVS_KEY_UBX "ubx_cfg"

#define UBX_GNSS_LIST(l)                                                       \
	l(UBX_GNSS_GPS, 0x00) l(UBX_GNSS_SBAS, 0x01) l(UBX_GNSS_GALILEO, 0x02)     \
		l(UBX_GNSS_BEIDOU, 0x03) l(UBX_GNSS_QZSS, 0x05)                        \
			l(UBX_GNSS_GLONASS, 0x06) l(UBX_GNSS_NAVIC, 0x07)
typedef enum ubx_gnss_e { UBX_GNSS_LIST(ENUM_V) } ubx_gnss_t;
...
#define GNSS_DESC_ITEM_LIST(l)                                                 \
	l(G + E + B + R) l(G + B + R) l(G + E + R) l(G + E + B) l(G + R) l(G + B)  \
		l(G + E)
extern const char *const gnss_desc[];
extern const size_t gnss_desc_items_count;

#define UBX_NAV_MODE_LIST(l)                                                   \
	l(UBX_MODE_PORTABLE, 0) l(UBX_MODE_PEDESTRIAN, 3)                          \
		l(UBX_MODE_AUTOMOTIVE, 4) l(UBX_MODE_SEA, 5) l(UBX_MODE_AIR_1G_MAX, 6) \
			l(UBX_MODE_AIR_2G_MAX, 7) l(UBX_MODE_AIR_4G_MAX, 8)
typedef enum ubx_nav_mode_e { UBX_NAV_MODE_LIST(ENUM_V) } ubx_nav_mode_t;

#define UBX_BAUD_RATE_LIST(l)                                                  \
	l(UBX_BAUD_9600, 9600) l(UBX_BAUD_38400, 38400) l(UBX_BAUD_57600, 57600)   \
		l(UBX_BAUD_115200, 115200) l(UBX_BAUD_230400, 230400)
typedef enum ubx_baud_rate_e { UBX_BAUD_RATE_LIST(ENUM_V) } ubx_baud_rate_t;
...
#define SAMPLE_RATE_ITEM_LIST(l)                                               \
	l(1_Hz) l(2_Hz) l(5_Hz) l(10_Hz) l(16_Hz) l(20_Hz)
extern const char *const sample_rates[];
extern const size_t sample_rates_items_count;
/**
 * @brief Message output rate enum.
 */
#define UBX_OUTPUT_RATES_LIST(l)                                               \
	l(UBX_OUTPUT_1HZ, 0x01) l(UBX_OUTPUT_2HZ, 0x02) l(UBX_OUTPUT_5HZ, 0x05)    \
		l(UBX_OUTPUT_10HZ, 0x0a) l(UBX_OUTPUT_16HZ, 0x10)                      \
			l(UBX_OUTPUT_20HZ, 0x14)
 ...
 */
#define UBX_MESSAGE_LIST(l)                                                    \
	l(UBX_MSG_DTM, 0x0A) l(UBX_MSG_GBS, 0x09) l(UBX_MSG_GGA, 0x00)             \
		l(UBX_MSG_GLL, 0x01) l(UBX_MSG_GRS, 0x06) l(UBX_MSG_GSA, 0x02)         \
			l(UBX_MSG_GST, 0x07) l(UBX_MSG_GSV, 0x03) l(UBX_MSG_RMC, 0x04)     \
				l(UBX_MSG_VTG, 0x05) l(UBX_MSG_ZDA, 0x08)
typedef enum ubx_message_e { UBX_MESSAGE_LIST(ENUM_V) } ubx_message_t;

#define UBX_MSG_TYPE_LIST(l)                                                   \
	l(MT_NONE, 0x00) l(MT_NAV_DUMMY, 0x01) l(MT_NAV_PVT, 0x02)                 \
		l(MT_NAV_ACK, 0x03) l(MT_NAV_NACK, 0x04) l(MT_NAV_ID, 0x05)            \
			l(MT_MON_GNSS, 0x06) l(MT_NAV_DOP, 0x07) l(MT_MON_VER, 0x08)       \
				l(MT_NAV_SAT, 0x09)
typedef enum ubx_msg_type_e { UBX_MSG_TYPE_LIST(ENUM_V) } ubx_msg_type_t;

extern const char *const ubx_msg_type_strings[];

#define UBX_EN_PIN_LEN 4

#define CFG_UBX_PRIVATE_ITEM_LIST(l) l(ubx_baud) l(ubx_msgout_sat)

#define CFG_UBX_PUBLIC_ITEM_LIST(l)                                            \
	l(ubx_output_rate) l(ubx_nav_mode) l(ubx_gnss)

#define CFG_UBX_ITEM_LIST(l) CFG_UBX_PUBLIC_ITEM_LIST(l)
...
enum cfg_ubx_item_e { CFG_UBX_ITEM_LIST(CFG_ENUM_UBX) };
...
uint8_t config_ubx_handle(void);
